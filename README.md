...
Массовый режим использует ускоренную проверку страниц и `itemprop`: backend проверяет наличие ссылок в HTML, но не выполняет глубокую загрузку каждого документа, чтобы большой список сайтов обрабатывался предсказуемо. Для детальной проверки конкретного сайта используется страница `/check`.
...
Запускает или продолжает массовую проверку. Если передать `{"reset": true}`, создаёт новый запуск с текущей датой.
